// all of my apis are built off of apis that are built off of the s4d examples api
...
// JSON.parse(JSON.stringify(db.all())).map(project => project.data).map(project => {
//     const newProject = JSON.parse(JSON.stringify(project))
//     if (newProject.image) {
//         const image = newProject.image
//         fs.writeFile(`./projects/uploadedImages/p${newProject.id}.uri`, image, "utf8", (err) => {
...
app.get('/api/projects/guidelines', async function(req, res) {
    ...
    res.json({
        "dontapprove": [
            "Do not approve projects that are just spam. (ex: same thing uploaded again with no changes or empty projects)",
            "Do not approve projects that misuse PenguinMod extensions to display content that is not allowed on PenguinMod.",
            "Do not approve projects with innapropriate thumbnails."
        ],
        "mightapprove": [
            "Projects containing horror or horror-like content should only be approved if it is mentioned somewhere on the project's page or in the project itself when running it."
        ]
    })
})
